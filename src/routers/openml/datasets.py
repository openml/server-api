...
async def list_datasets(  # noqa: PLR0913
    ...
    if user is None:
        ...
        if UserGroup.ADMIN in user_groups:
...
async def _get_processing_information(
    dataset_id: int, connection: AsyncConnection
...
async def update_dataset_status(
    ...
    user_groups = await user.get_groups()
    can_deactivate = dataset.uploader == user.user_id or UserGroup.ADMIN in user_groups
    if status == DatasetStatus.DEACTIVATED and not can_deactivate:
        raise HTTPException(
            status_code=HTTPStatus.FORBIDDEN,
            detail={"code": 693, "message": "Dataset is not owned by you"},
        )
    if status == DatasetStatus.ACTIVE and UserGroup.ADMIN not in user_groups:
        raise HTTPException(
            status_code=HTTPStatus.FORBIDDEN,
    ...
    if current_status is None or status == DatasetStatus.DEACTIVATED:
        await database.datasets.update_status(
            dataset_id, status, user_id=user.user_id, connection=expdb
        )
...
async def get_dataset(
    ...
    if not (
        dataset_file := await database.datasets.get_file(
            file_id=dataset.file_id, connection=user_db
        )
    ):
        error = _format_error(
