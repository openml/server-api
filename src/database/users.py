...
class User:
    ...
    first_name: str = ""
    last_name: str = ""
    _database: AsyncSession | None = None
    ...
    @classmethod
    async def fetch(cls, api_key: APIKey, user_db: AsyncSession) -> Self | None:
        user = await get_user(api_key=api_key, session=user_db)
        if user is not None:
            return cls(
    ...
    async def get_groups(self) -> list[UserGroup]:
        ...
        if self._database is None:
            msg = "`get_groups` can only be used when `session` is provided on instantiation."
...
async def exists_by_id(*, user_id: Identifier, session: AsyncSession) -> bool:
    row = await session.execute(
...
async def has_user_references(*, user_id: Identifier, expdb: AsyncSession) -> bool:
...
async def delete_user_rows(*, user_id: Identifier, userdb: AsyncSession) -> None:
