...
@pytest.mark.mut
async def test_attach_task_to_study(py_api: httpx.AsyncClient, expdb_session: AsyncSession) -> None:
...
async def test_attach_task_to_study_needs_owner(
    ...
    await expdb_session.execute(text("UPDATE study SET status = 'in_preparation' WHERE id = 1"))
    response = await _attach_tasks_to_study(
        study_id=1,
        task_ids=[2, 3, 4],
