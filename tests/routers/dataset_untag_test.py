...
async def test_dataset_untag_success(
    ...
    tag_present = await expdb_session.execute(
...
async def test_dataset_untag_tag_does_not_exist(expdb_session: AsyncSession) -> None:
    dataset_id = 1
    tag = "foo"

    with pytest.raises(TagNotFoundError) as e:
        await untag_dataset(dataset_id, tag, SOME_USER, expdb_session)
...
async def test_dataset_untag_tag_not_owned(expdb_session: AsyncSession) -> None:
    dataset_id = 1
    tag = "foo"
    await expdb_session.execute(
...
async def test_dataset_untag_admin_bypasses_ownership(expdb_session: AsyncSession) -> None:
    dataset_id = 1
    tag = "foo"
    await expdb_session.execute(
        text("INSERT INTO dataset_tag(id, tag, uploader) VALUES (:dataset_id, :tag, 1)"),
    ...
    await untag_dataset(dataset_id, tag, ADMIN_USER, expdb_session)
...
async def test_dataset_untag_dataset_does_not_exist(expdb_session: AsyncSession) -> None:
